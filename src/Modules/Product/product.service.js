...
export const updateProduct = async (req, res, next) => {
    try {
        ...
        if (updateData.variations) {
            ...
            let parsedVariations = [];
            try {
                parsedVariations = typeof updateData.variations === 'string' 
            ...
            if (Array.isArray(parsedVariations) && parsedVariations.length > 0) {
                ...
                for (let i = 0; i < parsedVariations.length; i++) {
                    ...
                    // رفع الصورة الأساسية للـ variation (لو موجودة)
                    let defaultImage = variant.defaultImage; // استخدم اللينك القديم لو موجود
                    if (req.files) {
                        const defaultImgFile = req.files.find(f => f.fieldname === `variation_${i}_defaultImage`);
                        if (defaultImgFile) {
                            defaultImage = await uploadToCloudinary(defaultImgFile.buffer);
                    ...
                    // رفع الصور الإضافية للـ variation (لو موجودة)
                    const variationImgs = variant.variationImgs || [];
                    if (req.files) {
                        const additionalImages = req.files.filter(f => 
                            f.fieldname.startsWith(`variation_${i}_image_`)
                        );
